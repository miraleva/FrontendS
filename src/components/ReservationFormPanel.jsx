...
const isDuplicateTurkishIdentityNumber = (
  ...
  if (
    currentPassenger?.nationality?.toUpperCase() !== "TR"
  ) {
    return false;
  }

  const currentIdentityNumber = normalizeIdentityNumber(
    currentPassenger?.identityNumber
  );

  if (!currentIdentityNumber) {
    return false;
  }
  ...
  return passengers.some((passenger, index) => {
    if (
      index === currentIndex ||
      passenger?.nationality?.toUpperCase() !== "TR"
    ) {
      return false;
    }

    return (
      normalizeIdentityNumber(passenger?.identityNumber) ===
...
export default function ReservationFormPanel({
  ...
  const validateReservationForm = () => {
    ...
    for (
      ...
      const errors = getPassengerErrors(
        guest,
        index,
        t,
